    ...
    private void bindViews(View frg) {
        ...
        spinnerSort.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                txv_Nationality.setText(lstNationalities.get(Pos));
        ...
        for (int i = 0; i < lstNationalities.size(); i++) {
            if (lstNationalities.get(i).equalsIgnoreCase(AppConfig.getInstance().mUser.getNationality())) {
...
//        if (edtGender.getText().toString().equalsIgnoreCase("Select Gender")) {
        if (txv_Nationality.getText().toString().equalsIgnoreCase(lstNationalities.get(lstNationalities.size() - 1))) {
//            AppConfig.getInstance().showErrorMessage(getContext(), "City is not selected");
